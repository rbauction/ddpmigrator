trigger PopulateGuidDdpPdfStamp on Loop__PDF_Stamp__c (before insert) {
    ...
    if (trigger.isBefore && trigger.isInsert) {
        ...
        for (Loop__PDF_Stamp__c l : trigger.new) {
            if (l.DDP_Migrator_Id__c != null)
                idsToCheck.add(l.DDP_Migrator_Id__c);
        }
        List<Loop__PDF_Stamp__c> existingRecords = [
            SELECT DDP_Migrator_Id__c
            FROM Loop__PDF_Stamp__c
            WHERE DDP_Migrator_Id__c IN :idsToCheck
        ];
        for (Loop__PDF_Stamp__c l : existingRecords)
